...
export class TestUsersCollection {
  ...
  sortByAge(sortDirection = 1): this {
    const sortFn = (a: UsersType, b: UsersType): number =>
      (a.age - b.age) * sortDirection;
  ...
  filterByNameAndAge(name: string, age: number): this {
    return this.filterByField("name", name).filterByField("age", age);
  }

  filterByNameAndCity(name: string, city: string): this {
    return this.filterByField("name", name).filterByField("city", city);
  }
